...
//#if UNITY_EDITOR
		public UnityWebResponse()
		{
           
		}

        public IEnumerator GetResponse(UnityWebRequest request)
        {
             Debug.Log("UnityWebResponse : uri " + request.uri.AbsoluteUri);

			WWW www = new WWW(request.uri.AbsoluteUri, request.body, request.headers);
